...
export default function UserRoleForm({ userRole, roles }: UserRoleFormProps) {
  ...
  // Pour le rôle et l'activité
  const [roleId, setRoleId] = useState(userRole?.role_id || '')
  const [isActive, setIsActive] = useState(userRole?.is_active ?? true)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (userRole) {
        // Mode Modification : mettre à jour le rôle et l'activité de l'utilisateur
        const { error: updateError } = await supabase
          .from('users')
          .update({ role_id: roleId, is_active: isActive })
          .eq('id', userRole.id) // userRole.id correspond à l'id de l'utilisateur dans la table users
        ...
        if (result.success) {
          setSuccess('Utilisateur créé et rôle attribué avec succès !')
          router.refresh()
          setTimeout(() => {
            router.push('/admin/users')
            router.refresh()
        ...
        <div className="flex items-center gap-3 py-2">
          <label className="flex items-center gap-2.5 cursor-pointer">
            ...
            <span className="text-sm font-semibold text-gray-700">
              Compte actif (permet la connexion au dashboard)
            </span>
          </label>
        </div>
